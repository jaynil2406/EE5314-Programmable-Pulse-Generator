...
void inithwssi()
{
    ...
    GPIO_PORTB_DIR_R |= 0x90;                        // make bits 4 and 7 outputs
       ...
       GPIO_PORTB_PCTL_R = GPIO_PCTL_PB7_SSI2TX | GPIO_PCTL_PB4_SSI2CLK |CS_NOT_MASK; // map alt fns to SSI2
...
// ***********************************STEP 7 (DC COMMAND )***********************************************************
      ...
      if(is_command("sine",4))
      {
          ...
          uint8_t   out = getArgInt(getString(0));
                 ...
                 uint8_t amp = getArgInt(getString(2));
                 uint8_t offset = getArgInt(getString(3));
          uint16_t i;
          //delta_phi = 2^32/100000 * freq;
          delta_phi = (42949)*freq;

         // delta_phi =   (40960)*freq;
          mode =0;
          TIMERON;
          for( i =0;i<4096;i++)
          {
             // D = (-384.416*volt)+1995.103;
              LUT1[i] = -384*amp*sin((2*M_PI*i)/4096)+1995+ offset + DAC_Aint ;
