...
if(mode==3)
{
    SSI2_DR_R = DAC_Aint+1995;
      SSI2_DR_R = DAC_Bint+1995;
}
if(mode==8)
{
    LDAC_NOT =0;
...
else  if(is_command("triangle",4))
{
    ...
    putsUart0("Triangle Wave");
    putsUart0("\r\n");
   // inttimer1A();
}
else if(is_command("cycles",2))
{
    putsUart0("Cycles Command");
    putsUart0("\r\n");
...
else if(is_command("gain",2))
{
    uint16_t i=0;
    mode = 9;
    swpfreqs = getArgFloat(getArgString(0));
    swpfreqe = getArgFloat(getArgString(1));
    for( i =0;i<4096;i++)
    {
        LUT1[i] = -384*((4*(sin((2*M_PI*i)/4096))))+1995  ;
    }
    delta_phi1 = ((1<<20)*(swpfreqs)/100000)*4096;
   // delta_phi2 = ((1<<20)*(swpfreqs)/100000)*4096;
    putsUart0("GAIN OF SINE");
    inttimer1A();
    inithwadc1();
    putsUart0("\nVoltage of OUTA:");
    uint16_t raw1=0,raw2=0 ;
    float Voltage1 =0 ,Voltage2=0,V=0;
    raw1 = readAdc0Ss3();
    Voltage1 = ( 3.3*(raw1 + 0.5 )/4096) ;
    inithwadc2();
    raw2 = readAdc0Ss3();
    Voltage2 = ( 3.3*(raw2 + 0.5 )/4096) ;
    V = Voltage1/Voltage2;
    sprintf(trial,"Voltage of OUTA:%f \r\n",V);
    putsUart0(trial);

}
else if(is_command("differential",1))
{
    mode=8;
    if(getArgInt(getArgString(0))==1)
    {
        uint16_t i=0;
        if(wave==1)
        {
            putsUart0("Differential Sine Wave ");
            putsUart0("\r\n");
            for(i=0;i<4096;i++)
            {
                if(i<2048)
                    LUT2[i] = -384*((amp1*(sin((2*M_PI*(i+2048)/4096)))))+1995  ;
                else
                    LUT2[i] = -384*((amp1*(sin((2*M_PI*(i-2048)/4096)))))+1995  ;
            }
        }
        if(wave==2)
        {
            putsUart0("Differential Square Wave");
            putsUart0("\r\n");
            i=0;
            for( i =0;i<4096;i++)
            {
                if(i<2048)
                    LUT2[i] =(-384.416)*(-amp1)+1995.103;
                else
                    LUT2[i] =(-384.416)*(amp1)+1995.103;
            }
        }
        if(wave==3)
        {
            putsUart0("Differential Sawtooth Wave");
            putsUart0("\r\n");
            i=0;
            for(i=0;i<4096;i++)
            {
                if(i<2048)
                    LUT2[i] = -384*( amp1/4096*(i+2048))+1995;
                else
                    LUT2[i] = -384*( amp1/4096*(i-2048))+1995;
            }
        }
        if(wave==4)
        {
            putsUart0("Differential Triangle Wave");
            putsUart0("\r\n");
            i=0;
            for(i=0;i<4096;i++)
            {
                if(i<2048)
                    LUT2[i] =(-384.416)*(amp1/2048*(4096-i))+1995.103;
                else
                    LUT2[i] =(-384.416)*(amp1/2048*i)+1995.103;
            }
        }
    }
...
else  if(is_command("squared",5))
{
      ...
      putsUart0("Square Wave With Programmable Duty Cycle");
      putsUart0("\r\n");
     //  inttimer1A();
}
else
    putsUart0("Trash Found(Please Enter a Valid Command");
    putsUart0("\r\n");
argCount =0;
        }
 }
