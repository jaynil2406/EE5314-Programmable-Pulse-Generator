...
if(out==1)
{
    ...
    if((phi>>20) < 2048)
    {
        DAC_valueA = (-384.416)*(amp+offset)+1995.103;
        DAC_valueA  = DAC_valueA + DAC_Aint;


        SSI2_DR_R = DAC_valueA;
    }
    if((phi>>20) >= 2048 && (phi>>20) < 4096)
      {
        DAC_valueA = (-384.416)*(-1*(amp-offset))+1995.103;
     DAC_valueA  = DAC_valueA + DAC_Aint;
          ...
          out = getArgInt(getString(0));
                    float freq = getArgFloat(getString(1));
                     amp = getArgInt(getString(2));
                      offset = getArgInt(getString(3));
mode =4;
delta_phi = ((1<<20)*(freq*2)/100000)*4096;
                    inttimer1A();
