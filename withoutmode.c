...
uint8_t mode=100;
float offset,amp;
uint32_t delta_phi,delta_phi1;
uint16_t DAC_valueA,DAC_valueB,DAC_valueA1,DAC_valueB1;
//-----------------------------------------------------------------------------
// Subroutines
//-----------------------------------------------------------------------------
...
// ***********************************STEP 7 (DC COMMAND )***********************************************************
      ...
      if(is_command("stop",0))
      {
          mode = 3;
      }
      if(is_command("sine",4))
      {
          uint16_t i;
           amp = getArgInt(getString(2));
           offset = getArgFloat(getString(3));
           out = getArgInt(getString(0));
                         float freq = getArgFloat(getString(1));
                         if(out==1)
                         {
          for( i =0;i<4096;i++)
                 {
                    // D = (-384.416*volt)+1995.103;
                     LUT1[i] = -384*((amp*(sin((2*M_PI*i)/4096)))+offset)+1995  ;
          ...
          for( i =0;i<4096;i++)
                     {
                        // D = (-384.416*volt)+1995.103;
                         LUT2[i] = -384*((amp*(sin((2*M_PI*i)/4096)))+offset)+1995  ;
         ...
         // delta_phi = (42949)*freq;
          delta_phi = ((1<<20)*(freq)/100000)*4096;
         // mode =0;
      ...
      if(is_command("square",4))
      {
          uint16_t i =0;
          out = getArgInt(getString(0));
                    float freq = getArgFloat(getString(1));
                     amp = getArgFloat(getString(2));
                      offset = getArgFloat(getString(3));
                      if(out==1)
                      {
                      for( i =0;i<4096;i++)
                      {
                             if(i<2048)
                             {
                                 LUT1[i] =(-384.416)*(amp+offset)+1995.103;
                             }
                             else
                             {
                                 LUT1[i] =(-384.416)*(-amp+offset)+1995.103;
                      ...
                      for( i =0;i<4096;i++)
                      {
                          if(i<2048)
                          {
                              LUT2[i] =(-384.416)*(amp+offset)+1995.103;
                          }
                          else
                          {
                              LUT2[i] =(-384.416)*(-amp+offset)+1995.103;
              ...
              uint16_t i=0;
               ...
               out = getArgInt(getString(0));
                             ...
                             if(out==1)
                             {
                                 for(i=0;i<4096;i++)
                                 {
                                     LUT1[i] = -384*(offset + amp/4096*i)+1995;
                                 }
                             }
                             if(out==2)
                             {
                                 for(i=0;i<4096;i++)
                                 {
                                     LUT2[i] = -384*(offset + amp/4096*i)+1995;
                                 }
                             }
                             delta_phi = ((1<<20)*(freq)/100000)*4096;
                             inttimer1A();
          }
      if(is_command("cycles",1))
      {
          cycles = getArgInt(getString(0));
      }
           argCount =0;
